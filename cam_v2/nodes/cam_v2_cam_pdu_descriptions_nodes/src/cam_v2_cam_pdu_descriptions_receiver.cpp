...
wind::wind_ros::Receiver_cam_v2_cam_pdu_descriptions::start() {
	...
	decoder_ = new wind::decoder_cam_v2_cam_pdu_descriptions::WerDecoder(slogger_, debug_);
			
    #if WIND_ROS_VERSION == 1      	  
      publisher_        = new ros::Publisher(node_handle_.advertise<cam_v2_cam_pdu_descriptions_msgs::CAM>(topic_, 0));
      mirror_publisher_ = new ros::Publisher(node_handle_.advertise<cam_v2_cam_pdu_descriptions_msgs::CAM>(SENDER_ROS_TOPIC, 0));	  	 
    #else
      publisher_         = create_publisher<cam_v2_cam_pdu_descriptions_msgs::msg::CAM>(topic_, 10);
      mirror_publisher_  = create_publisher<cam_v2_cam_pdu_descriptions_msgs::msg::CAM>(SENDER_ROS_TOPIC, 10);
    #endif
}
...
{	
    ...
    if(messageId == 2 && protocolVersion == 2) {
        ...
        #if WIND_ROS_VERSION == 1    
          cam_v2_cam_pdu_descriptions_msgs::CAM msg;
        #else
           cam_v2_cam_pdu_descriptions_msgs::msg::CAM msg;
        #endif
		
		bool decoded = decoder_->decode(&msg, &buffer[offset]);
        if(decoded)
        {
            slogger_->print() << "Incoming CAM: msgId("      << messageId   << ":" << protocolVersion << 
			                    ") stationId("      << stationId << 
                                ") test("           << bit0      << 
                                ") mirror("         << bit1      << 
                                ")";
            if(bit1) {
                slogger_->print() << "Publishing CAM on topic " << SENDER_ROS_TOPIC;
                mirror_publisher_->publish(msg);
            } else {
                slogger_->print() << "Publishing CAM on topic " << MAGENTA << topic_ << RESET;
